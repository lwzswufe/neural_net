...
class AutoEncoder(object):
    def __init__(self, param, is_plot=False):
        self.param = param
        self.sess = ''
    ...
    def train(self):
        ...
        encoded = tf.layers.dense(en2, 16, name='encoded')
        w4 = tf.add(y, y, name='encoded2')

        # decoder
        # layers is not in the graph
        ...
        decoded = tf.layers.dense(de2, self.param.x_dim, tf.nn.sigmoid, name='decoded')
        ...
        for step in range(self.param.train_times):
            ...
            if step % 100 == 0 and self.is_plot:     # plotting
        ...
        if self.is_plot:
            plt.ioff()
            encodes = encoded.eval(session=self.sess, feed_dict={tf_x: self.data.train.images})
            train_data = MyDataSet.DataSet(images=encodes, labels=self.data.train.labels)
            with open('D:\\Cache\\encodes_mnist.pic', 'wb') as f:
...
if __name__ == '__main__':
    ...
    #	load_data.save(c, fn)
    param = Param(train_times=200, learning_rate=0.002)
    ae = AutoEncoder(param, is_plot=True)
    ae.input_data(data=None)
    ae.train()
