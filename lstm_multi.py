...
class loss_layer(object):
    ...
    def loss(self, pred, label):
        if label is None:
            ...
            return np.sum((np.array(pred) - np.array(label)) ** 2)  # 误差函数  **=^
...
class LstmNode(object):
    ...
    def back_propagation(self, top_diff_h, top_diff_s, loss_y):  # 后向计算
        ...
        dh = np.dot(self.param.wy.T, loss_y) + top_diff_h
        dhh = dh * self.state.o
...
class LstmNetwork(object):
    def __init__(self, y_list, x_list, cell_dim, y_dim=1, sampling=False):
        ...
        self.y_dim = y_dim
        if self.y_dim != max(y_list) + 1:
            self.y_dim = int(max(y_list) + 1)
        self.lstm_param = LstmParam(cell_dim, x_dim, self.y_dim, id=999)
        self.lstm_node_list = list()  # 节点(Node)
        # input sequence
        self.loop_id = 0
        self.x_list = x_list
        self.y_list = y_list
        self.true_list = np.array(y_list)
        self.y_predict = list()
        ...
        self.sample_flag = 0
        self.err = ErrorStat(class_num=self.y_dim)
    ...
    def train(self, lr=False, maxloop=False, min_loos_diff=False, attenuation=False,
        ...
        loss_diff = 1
        loss = 0
        y_predict = np.zeros(self.sample_train_num)
        lr_var = 1.0
        y_list = self.y_list
        x_list = self.x_list
        self.net_initial()
        ...
        while self.cur_iter < maxloop and loss_diff > min_loos_diff:  # 迭代次数
            ...
            # self.snapShot()
            if self.sampling:
                y_predict[self.sample_flag] = self.y_predict[-1].argmax()
            if not self.sampling or self.sample_flag == self.sample_train_num - 1:
                self.err.stat_err(self.true_list, y_predict)
        ...
        return loss_list, lr_list, self.err
    ...
    def predict(self, x_list):
        ...
        for i in range(len(x_list)):
            y_predict.append(self.y_predict[i].argmax())
        return y_predict
    ...
    def predict_sample(self, x_list):
        ...
        for i in range(len(x_list) - self.sample_x_len):
            ...
            y_predict.append(self.y_predict[-1].argmax())
        return y_predict
    ...
    def reset_loop(self):
        ...
        if self.sampling:
            ...
            self.sample_flag += 1
            if self.sample_flag >= self.sample_train_num:
                self.sample_flag = 0
    ...
    def sample_initial(self, x_list, y_list):
        ...
        while flag < self.sample_train_num:
            ...
            k = int(y_list[idx + self.sample_x_len])
            if class_num[k] >= self.sample_train_num / self.y_dim:
                continue
            else:
                class_num[k] += 1.0
            ...
            for j in range(self.sample_x_len):
                if j >= self.sample_x_len - self.sample_y_len:
                    y0 = np.zeros([self.y_dim, 1])
                    y0[int(y_list[idx + j])] += 1
                    y_sample.append(y0)
                else:
                    y_sample.append(None)
            self.true_list.append(y_list[idx + self.sample_x_len - 1])
...
class ErrorStat(object):
    def __init__(self, class_num=3):
        self.err_1_list = list()
        self.err_2_list = list()
        self.class_num = class_num
        for _ in range(class_num):
            self.err_1_list.append(list())
            self.err_2_list.append(list())

    def stat_err(self, y_0, y_pre):
        n = len(y_0)
        common = np.zeros([n, n])
        for i in range(n):
            common[int(y_0[i]), int(y_pre[i])] += 1
        for label in range(self.class_num):
            err_1 = common[label, label] / max(1, sum(common[label, :]))
            err_2 = common[label, label] / max(1, sum(common[:, label]))
            self.err_1_list[label].append(err_1)
            self.err_2_list[label].append(err_2)

    def plot(self, err_type=1):
        legend = list()
        for label in range(self.class_num):
            if err_type == 1:
                plt.plot(self.err_1_list[label])
            else:
                plt.plot(self.err_2_list[label])
            legend.append('class_' + str(label) + '_err')
        plt.legend(legend)
        plt.show()
...
def example():
    x_dim = 50
    y_list = [None, None, [[0.2], [0.3], [0.5]], [[0.1], [0.5], [0.4]], [[0.7], [0.2], [0.1]]]
    np.random.seed(0)
    x_list = [np.random.random([x_dim, 1]) for _ in y_list]
    lstm_net = LstmNetwork(x_list=x_list, y_list=y_list, cell_dim=10, y_dim=3)
